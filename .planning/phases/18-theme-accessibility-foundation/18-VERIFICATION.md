...
| 2   | Admin sidebar uses the same theme variables as the rest of the app (no hardcoded Tailwind color classes) | ✓ VERIFIED | admin-sidebar.tsx uses var(--brass), var(--bg-_), var(--text-_), var(--border-\*); zero zinc/amber classes   |
