    ...
    <div className="mx-auto max-w-[720px] px-4 py-8 sm:py-12">
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-2">
          ...
          {order.type === "MINT" && order.mintPrice && (
            <DetailRow label="Mint Price">
              <span className="font-mono tabular-nums">
                ${(Number(order.mintPrice) / 1e18).toFixed(4)}
              </span>
            </DetailRow>
          )}
          ...
          {order.trackingId && (
            <DetailRow label="Tracking">
              <a
                href={`https://www.ups.com/track?loc=en_US&tracknum=${order.trackingId}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 font-mono text-xs transition-colors hover:underline"
                style={{ color: "var(--brass)" }}
              >
                {order.trackingId}
                <svg
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            onClick={() =>
              toast.info("Coming Soon", {
                description:
                  "Problem reporting is under development. Stay tuned!",
              })
            }
