...
export type PriceHistory = Record<AmmoType, PricePoint[]>;
...
export function createAmmoPriceHistory(
  ...
  // Shared market factor: simulates broad pressure from inputs / retail competition / inventory.
  let commonFactor = randomBetween(rng, -0.01, 0.01);
  ...
  // Self-defense ammo is more stable and sticky than practice ammo.
  const params: Record<
    AmmoType,
    {
  ...
  for (let i = 0; i < days; i++) {
    ...
    // Common factor mean-reverts so the whole market doesn't drift away.
    ...
    for (const ammo of ammoTypes) {
      ...
      const priceCents = Math.max(1, Math.round(base * (1 + deviation)));

      history[ammo].push({ date: formatDate(date), priceCents });
    }
  }
